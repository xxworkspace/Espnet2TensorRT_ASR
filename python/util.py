...
def position_wise(network,input,seql,odim,dtype,model_dict,prefix):
  creator = trt.IPluginRegistry.get_plugin_creator("PositionWise_TRT","")
...
def self_attention(network,input,n_head,odim,mask,model_dict,prefix):
  ...
  creator = trt.IPluginRegistry.get_plugin_creator("SelfAttention_TRT","")
  args = trt.PluginField("args",[n_head,odim,mask], trt.PluginFieldType.INT32)
...
def final_slice(network,input):
  creator = trt.IPluginRegistry.get_plugin_creator("FinalSlice_TRT","")
